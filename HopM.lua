...
do
    ...
    -- // Constructor
    function HopM.new(Data)
        -- // Default and assert
        Data = Data or {}
        assert(typeof(Data) == "table", "invalid type for Data (expected table)")

        -- // Create the object
        local self = setmetatable({}, HopM)

        -- // Load the defaults
        self.Data = Utilities.CombineTables(Utilities.DeepCopy(HopM.DefaultData), Data)
    ...
    function HopM:ValidateServer(Server)
        -- // Vars
        local Data = self.Data
    ...
    function HopM:GetServerDataURL(Url)
        -- // Vars
        local Data
    ...
    function HopM:GetMassServerListCreate(PlaceId, Count)
        -- // Vars
        local Data = self.Data
    ...
    function HopM:GetMassServerList(PlaceId)
        -- // Default and assert
        PlaceId = PlaceId or game.PlaceId
        assert(typeof(PlaceId) == "number", "invalid type for PlaceId (expected number)")
    ...
    function HopM:GetServerList(PlaceId)
        -- // Default and assert
        PlaceId = PlaceId or game.PlaceId
        assert(typeof(PlaceId) == "number", "invalid type for PlaceId (expected number)")
    ...
    -- // Hop servers
    function HopM:Hop(PlaceId, JobId, Script)
        -- // Default and assert
        PlaceId = PlaceId or game.PlaceId
        Script = Script or ""
        assert(typeof(PlaceId) == "number", "invalid type for PlaceId (expected number)")
